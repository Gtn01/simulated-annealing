    ...
    public abstract class EditorHelper : Common.EditorUtil.BaseEditorHelper
    {
        ...
        public static void BannerFB()
        {
            if (!Util.Constants.isPro && Util.Constants.SHOW_FB_BANNER)
            {
                ...
                {
                    EditorGUILayout.HelpBox("'File Browser PRO' is not installed!" + System.Environment.NewLine + "For Linux, UWP (WSA), Windows (IL2CPP), PlayMaker actions and extended support, please get it from the Unity AssetStore.", MessageType.Info);
